...
function handleRegistro(event) {
    ...
    mostrarMsg(successEl, `🎉 ¡Cuenta creada exitosamente! Ahora inicia sesión.`);
...
function verificarSesion() {
    ...
    if (sesion) {
        ...
        if (btnLogin) {
            ...
            btnLogin.onclick = () => window.location.href = 'dashboard.html';
        }
        if (btnReg) {
            btnReg.textContent = `👤 ${sesion.nombre.split(' ')[0]}`;
            btnReg.onclick = () => window.location.href = 'dashboard.html';
        }
    }
}
